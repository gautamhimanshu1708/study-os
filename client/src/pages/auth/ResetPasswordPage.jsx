...
import { Lock, Eye, EyeOff, Zap, CheckCircle2, ArrowLeft, RefreshCw, Mail } from 'lucide-react';
...
const ResetPasswordPage = () => {
  ...
  const [isEditingEmail, setIsEditingEmail] = useState(!emailParam && !token);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await resetPassword(
        token ? { password } : { email: email.trim(), otp: fullOtp, password },
        token
      );
      setSuccess(true);
  ...
  const handleResend = async () => {
    ...
    setResending(true);
    try {
      await resendOtp({ email: email.trim(), type: 'forgot-password' });
      toast.success('A new 6-digit password reset OTP has been sent to your email.');
      setCountdown(60);
      setCanResend(false);
      setOtp(['', '', '', '', '', '']);
          ...
          {!success ? (
            <>
              ...
              {!token && (
                <>
                  {/* Email Pill */}
                  <div className="mb-4 p-3 bg-base-950/80 border border-border/80 rounded-xl flex items-center justify-between gap-2">
                ...
                <div className="mt-4 text-center">
                  <p className="text-xs text-text-secondary">
                    ...
                    {canResend ? (
                      ...
                      >
                        <RefreshCw size={12} className={resending ? 'animate-spin' : ''} /> Resend OTP
                      </button>
                    ) : (
